...
We notice that in \citet{Fujimoto2019} they present a Q-learning approach, for which
...
The critic network stays untouched.
To sample actions for the target values, we introduce a variational autoencoder $G_w$,
to generate actions $\hat{a}$ sampled close to the distribution of actions in the dataset.
Afterwards, we perturb the action samples to generate actions $a$ using a perturbation model $\xi$.
This perturbation model is trained the same way as the previously presented actor network,
i.e. to maximize the CVaR of the estimated return distribution.
...
algorithm will imitate more the actions in the dataset.
...
Algorithm is implemented same was as the one for off-policy RL, and hence shares almost all details
as the ones introduced in \ref{sec:technical_details}.
We focus here on the differences or new additions.
Here, we do not need to account for exploration so there is no such behavior policy.
