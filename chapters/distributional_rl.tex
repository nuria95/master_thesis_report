...
\begin{equation}
    \prod_ {W_1} Z \coloneqq  \underset{Z_\theta \in \mathcal{Z_Q}}{\arg \min } W_1(Z,Z_\theta) \label{eq:discrete_pdf}
\end{equation}
\todo{get the W1 next to the prod not below}
...
$W_Y(Y,U)$ are given by $\theta_i = F_Y^-1(\hat\tau_i)$, where $\hat\tau_i=\frac{\tau_{i-1}+\tau_i}{2}$.
...
for quantile regression, creating the quantile regression temporal differnce learning algorithm:
